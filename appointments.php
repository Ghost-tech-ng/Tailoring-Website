        ...
        <div class="container-fluid py-4">
            <div class="row">
                <div class="container-fluid">
                    <h1>Appointments</h1>
                    <button type="button" class="btn btn-primary mb-3" data-bs-toggle="modal" data-bs-target="#addAppointmentModal">
                        Add Appointment
                    </button>
                        ...
                        <table class="table table-striped">
                            ...
                            <tbody>
                                <?php foreach ($appointments as $appointment): ?>
                                    <tr>
                                        <td><?php echo htmlspecialchars($appointment['customer_id']); ?></td>
                                        <td><?php echo htmlspecialchars($appointment['appointment_date']); ?></td>
                                        <td><?php echo htmlspecialchars($appointment['notes']); ?></td>
                                        <td><?php echo $appointment['is_completed'] ? 'Completed' : 'Pending'; ?></td>
                                        <td>
                                            <!-- Edit Button -->
                                            <button type="button" class="btn btn-warning" data-bs-toggle="modal" data-bs-target="#editAppointmentModal<?php echo $appointment['id']; ?>">
                                                Edit
                                            </button>
                                            <!-- Complete Button (only if not completed) -->
                                            <?php if (!$appointment['is_completed']): ?>
                                                <form method="post" action="complete_appointment.php" style="display:inline;">
                                                    <input type="hidden" name="id" value="<?php echo $appointment['id']; ?>">
                                                    <button type="submit" class="btn btn-success" onclick="return confirm('Mark this appointment as completed?');">Complete</button>
                                                </form>
                                            <?php endif; ?>
                                        </td>
                                    </tr>
                                            ...
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-body">
                                                    <form method="post" action="edit_appointment.php">
                                                        <input type="hidden" name="id" value="<?php echo $appointment['id']; ?>">
                                                        <div class="mb-3">
                                                            <label for="editCustomerID<?php echo $appointment['id']; ?>" class="form-label">Customer Name</label>
                                                            <input type="text" class="form-control" id="editCustomerID<?php echo $appointment['id']; ?>" name="customer_id" value="<?php echo htmlspecialchars($appointment['customer_id']); ?>" required>
                                                        </div>
                                                        <div class="mb-3">
                                                            <label for="editAppointmentDate<?php echo $appointment['id']; ?>" class="form-label">Appointment Date & Time</label>
                                                            <input type="datetime-local" class="form-control" id="editAppointmentDate<?php echo $appointment['id']; ?>" name="appointment_date" value="<?php echo date('Y-m-d\TH:i', strtotime($appointment['appointment_date'])); ?>" required>
                                                        </div>
                                                        <div class="mb-3">
                                                            <label for="editNotes<?php echo $appointment['id']; ?>" class="form-label">Notes</label>
                                                            <textarea class="form-control" id="editNotes<?php echo $appointment['id']; ?>" name="notes" rows="3"><?php echo htmlspecialchars($appointment['notes']); ?></textarea>
                                                        </div>
